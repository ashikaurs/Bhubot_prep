...
def chat():
    ...
    context = f"""Season:{data.get('season','Kharif')}, Location:{data.get('location','Karnataka,India')}, Land:{land_size}, Budget:{budget}
pH:{soil_params.get('ph',6.5)}, N:{soil_params.get('nitrogen','Medium')}, P:{soil_params.get('phosphorus','Medium')}, K:{soil_params.get('potassium','Medium')}
OC:{soil_params.get('organic_carbon',0.5)}%, EC:{soil_params.get('ec',0.4)}dS/m
Zn:{soil_params.get('zinc','Medium')}, Fe:{soil_params.get('iron','Medium')}, Mn:{soil_params.get('manganese','Medium')}, Cu:{soil_params.get('copper','Medium')}, B:{soil_params.get('boron','Medium')}, S:{soil_params.get('sulphur','Medium')}
Moisture:{mock_sensor['moisture']}%, Temp:{mock_sensor['temperature']}°C"""

    target = f"for {crop_instruction}" if crop else "for general soil improvement"

    prompt_part1 = f"""You are an expert agricultural advisor for Indian farmers. Respond ONLY in {language}. Complete all sentences fully.

SOIL DATA: {context}
{"Crop: " + crop_instruction if crop else "No crop selected — give general recommendations."}

Generate these sections:

📊 **Soil Health Score: [X]/100**
- pH/N/P/K/OC/Micronutrients with scores and status
- 🎯 Main Limiting Factor and consequence {target}

🧪 **Soil Analysis {target}**
- pH, N/P/K, OC, EC, estimated soil type, moisture status

{"🌾 **Top 3 Recommended Crops** with reasons" if not crop else ""}

🔴 **High Priority Actions** (2 items with yield impact warnings)
🟡 **Medium Priority Actions** (2 items)
🟢 **Low Priority Actions** (2 items)

🌿 **{"Fertilizer Plan for " + crop if crop else "General Fertilizer Recommendations"}**
- Organic, Chemical (if needed), Micronutrients with quantities per acre and timing

Keep response under 300 words. Complete every sentence fully in {language}."""

    prompt_part2 = f"""You are an expert agricultural advisor for Indian farmers. Respond ONLY in {language}. Complete all sentences fully.

SOIL DATA: {context}
{"Crop: " + crop_instruction if crop else "No crop selected — give general guidance."}

Generate these sections:

💧 **{"Irrigation Plan for " + crop_instruction if crop else "General Irrigation Guidance"}**
- Soil type, moisture status, recommended method
{"- Stage-wise water requirements (land prep, germination, vegetative, flowering, maturity)" if crop else "- General water needs and drainage warnings"}
- ⚠️ Water stress warning

💰 **Cost Estimate per Acre**
- Seeds, organic inputs, fertilizers, micronutrients, irrigation, labor
- **Total: ₹[range]**, expected yield improvement %, estimated return

💡 **Practical Tips for {data.get('location','your region')}**
- 2 local low-cost tips ✅, 1 common mistake warning ⚠️, 1 action for this week ✅

📅 **{"Week-by-Week Action Plan" if crop else "Soil Preparation Action Plan"}**
- Week 1–2, Week 3–4, {"Week 5–8, At Harvest" if crop else "Week 5–6, Before Sowing"}
            ...
            messages=[
                {
                    ...
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": """Is this an Indian soil health card with parameters like pH, N, P, K?
If NOT a soil card, respond ONLY: {"error": "not_soil_card"}

If YES, extract values and respond ONLY with this JSON:
{"ph":6.5,"nitrogen":"Medium","phosphorus":"Medium","potassium":"Medium","organic_carbon":0.5,"sulphur":"Medium","zinc":"Medium","iron":"Medium","manganese":"Medium","copper":"Medium","boron":"Medium","ec":0.4}

Rules: ph/organic_carbon/ec = numbers. All nutrients = "Low"/"Medium"/"High". Use defaults if missing. Return ONLY JSON, nothing else."""
                        }
                    ]
                }
            ],
        ...
        return jsonify({"success": True, "soil_params": parsed})

    except json.JSONDecodeError:
        return jsonify({"success": False, "error": "Could not parse soil parameters from image."}), 500
